...
function ScoreDots({score}:{score?:number}) {
  const label=score===undefined?"未评分":`${score} 分`;
  return <span className="score-dots" title={label} aria-label={label}>{[1,2,3].map(point=><i key={point} className={score!==undefined&&score>=point?"on":""}/>)}</span>;
}
...
export default function Home() {
  ...
  return (
    <main>
      ...
      {view === "results" ? <Results key={queryId} profiles={profiles} candidates={candidates} reviews={reviews} teamReviews={teamReviews} queryId={queryId} queryName={queryProfile.name} intent={intent} evaluator={evaluator} llmRuns={llmRuns} onChangeQuery={id=>{changeQuery(id);const profile=profiles.find(p=>p.id===id);setIntent(profile?.intent||"该用户没有提供 Current_Social_Intent")}} onOpenCandidate={id=>{const index=candidates.findIndex(c=>c.id===id);if(index>=0){setCandidateIndex(index);setInspectedSource("all");setView("review")}}} /> : <>
        ...
        <div className="workspace">
          <aside className="queue">
            ...
            <div className="candidate-list">{candidates.map((c,i)=>{
              ...
              return <button key={c.id} className={i===candidateIndex?"active":""} onClick={()=>{setCandidateIndex(i);setInspectedSource(source=>source==="all"?"all":"mine")}}><span className="avatar">{avatarNumber(c.name)}</span><span><b>{c.name}</b><small>{c.role}</small></span>{score===undefined?<em>—</em>:<em className="score">{score}</em>}</button>
            })}</div>
          </aside>
...
function Results({profiles,candidates,reviews,teamReviews,queryId,queryName,intent,evaluator,llmRuns,onChangeQuery,onOpenCandidate}:{profiles:Profile[];candidates:Profile[];reviews:Record<string,Review>;teamReviews:TeamReview[];queryId:string;queryName:string;intent:string;evaluator:Evaluator|null;llmRuns:LLMRun[];onChangeQuery:(id:string)=>void;onOpenCandidate:(id:string)=>void}) {
  ...
  return <section className="results-page parallel-page">
    <div className="results-title"><div><div className="step-label">PARALLEL EVALUATION</div><h1>多评审结果对比</h1><p>选择最多 4 个评分来源并排查看 · 点击候选人进入 Profile</p></div><div className="results-actions"><label><span>QUERY USER</span><select value={queryId} onChange={e=>onChangeQuery(e.target.value)}>{profiles.map(profile=><option key={profile.id} value={profile.id}>{profile.name} · {profile.role}</option>)}</select></label><button className="dark export-top" onClick={exportSelected}>导出当前对比 JSON ↓</button></div></div>
